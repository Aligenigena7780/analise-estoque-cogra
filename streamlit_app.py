...
def grafico_ultimos_6_meses(df_vendas: pd.DataFrame, ano_ref: int, mes_ref: int) -> pd.DataFrame:
    ...
    base["ordem"] = base["ano_mes"].apply(lambda x: chaves.index(x))
    base = base.sort_values("ordem")

    # garantir meses sem venda
...
st.markdown('<div class="sub-title">Bloco 1 — Análise da Cogra</div>', unsafe_allow_html=True)
...
c1.metric("Faturamento Bruto", fmt_brl(kpis_atual["faturamento_bruto"]), fmt_var(variacoes["faturamento_bruto"]))
c2.metric("Faturamento Líquido", fmt_brl(kpis_atual["faturamento_liquido"]), fmt_var(variacoes["faturamento_liquido"]))
c3.metric("Lucro", fmt_brl(kpis_atual["lucro"]), fmt_var(variacoes["lucro"]))
c4.metric("Margem", fmt_pct(kpis_atual["margem"]), fmt_var(variacoes["margem"]))
c5.metric("GMROII", f'{kpis_atual["gmroii"]:.2f}'.replace(".", ","), fmt_var(variacoes["gmroii"]))
c6.metric("Estoque Total", fmt_brl(kpis_atual["estoque_total"]))
